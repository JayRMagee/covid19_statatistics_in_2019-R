...
data = COVID19_line_list_data

describe(data) #hmisc command

#cleaned up death data
data$death_dummy = as.integer(data$death != 0)
#run unique(data$death_dummy) to make sure that it worked
...
#p-value < 0.01 so we reject null hypothesis, so this is
...
#p-value = 0.002 < 0.01, so this is statistically significant
